...
document.getElementById("increase").addEventListener("click", () => {
  const increaArr = arr.slice().sort((a, b) => a - b);
...
document.getElementById("change").addEventListener("click", () => {
  if (
    ...
    +position1.value < 0 ||
    +position2.value < 0 ||
    +position1.value > arr.length - 1 ||
    +position2.value > arr.length - 1
  )
  ...
  document.getElementById("result6").value = arr.join("  ");
});

// 9. Nhập thêm 1 mảng số thực, tìm xem trong mảng có bao nhiêu số nguyên?
const real = document.getElementById("real-number");
let arrReal = [];
document.getElementById("input-real-number").addEventListener("click", () => {
  if (!real.value) return;
  const realValue = +real.value;
  arrReal.push(realValue);
  real.value = "";
  document.getElementById("result9").value = arrReal.join("  ");
});

document.getElementById("calculate-integer").addEventListener("click", () => {
  const totalInteger = arrReal.filter((cur) => cur === Math.floor(cur));

  document.getElementById("total-number-integer").value = totalInteger.length;
});
