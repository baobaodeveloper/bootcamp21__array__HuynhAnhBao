...
document.getElementById("combine").addEventListener("click", () => {
  ...
  if (positive.length < negative.length) {
    combine = `Số lượng số âm là ${negative.length} lớn hơn số lượng số dương ${positive.length}`;
  } else if (positive.length > negative.length) {
    combine = `Số lượng số dương là ${positive.length} lớn hơn số lượng số âm ${negative.length}`;
  } else {
